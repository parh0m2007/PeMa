#!/bin/bash
# SportCal launcher — сборка + бэкенд + приложение одной кнопкой

APP_DIR="$(cd "$(dirname "$0")" && pwd)"
API_DIR="$APP_DIR/api"
BUILD_DIR="$APP_DIR/build-local"
QT_APP="$BUILD_DIR/QtCalendarApp.app"
...
if [ $? -ne 0 ]; then
    echo "❌ Ошибка сборки. Запустите SportCal.command из Терминала чтобы увидеть детали."
...
sleep 3
while pgrep -f "QtCalendarApp" > /dev/null 2>&1; do
    sleep 2
done
